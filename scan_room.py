...
SWEEP_SPEED_DPS = 3.0
SWEEP_SEGMENTS = [(0.0, 360.0), (360.0, 0)]

PRE_SWEEP_SETTLE_SEC = 0.6
POST_SWEEP_SETTLE_SEC = 0.2
BOUNDARY_SKIP_SAMPLES = 0
...
MOUNT_ROLL_CORR_DEG = -0.9
MOUNT_PITCH_CORR_DEG = 0.0
MOUNT_YAW_CORR_DEG = 0.0
...
def arduino_cmd(ser: serial.Serial, cmd: str, retries: int = 1, response_timeout: float = 2.0):
    last_error = None
...
def goto_angle_blocking(arduino, target_deg: float, acceptable_deg: float = 1.0, timeout: float = 40.0):
    try:
...
def sweep_return_to_zero(arduino, speed_dps: float = 3.0):
...
def make_mount_correction_matrix() -> np.ndarray:
    roll_R = rotation_matrix("x", MOUNT_ROLL_CORR_DEG)
    pitch_R = rotation_matrix("y", MOUNT_PITCH_CORR_DEG)
    yaw_R = rotation_matrix("z", MOUNT_YAW_CORR_DEG)
...
def lidar_sample_to_xyz(distance_m: float, azimuth_deg: float, tilt_deg: float):
    ...
    corrected_tilt_deg = TILT_SCALE * tilt_deg + TILT_OFFSET_DEG
    tilt_angle = TILT_SIGN * (corrected_tilt_deg + TILT_ZERO_OFFSET_DEG)
    tilt_R = rotation_matrix(TILT_ROT_AXIS, tilt_angle)

    mount_corr_R = make_mount_correction_matrix()

    p = tilt_R @ (mount_corr_R @ (scan_R @ local_beam) + SENSOR_OFFSET)
...
def run_continuous_sweep(arduino, sdk_reader: UltraSimpleReader, start_deg: float, end_deg: float, speed_dps: float):
...
def main():
    print_ports()
    print(f"SWEEP_SPEED_DPS = {SWEEP_SPEED_DPS}")
    print(f"SCAN_ZERO_OFFSET_DEG = {SCAN_ZERO_OFFSET_DEG}")
    print(f"SCAN_SIGN = {SCAN_SIGN}")
    print(f"TILT_SIGN = {TILT_SIGN}")
    print(f"TILT_SCALE = {TILT_SCALE}")
    print(f"TILT_OFFSET_DEG = {TILT_OFFSET_DEG}")
    print(f"MOUNT_ROLL_CORR_DEG = {MOUNT_ROLL_CORR_DEG}")
    print(f"MOUNT_PITCH_CORR_DEG = {MOUNT_PITCH_CORR_DEG}")
    print(f"MOUNT_YAW_CORR_DEG = {MOUNT_YAW_CORR_DEG}")
    print()
    ...
    try:
        ...
        if USE_HOME_AT_START:
            ...
            print("\nPlace the rig at your intended center/zero before continuing.")
        ...
        for segment_id, (start_deg, end_deg) in enumerate(SWEEP_SEGMENTS):
            ...
            arduino, lidar_samples, encoder_samples = run_continuous_sweep(
